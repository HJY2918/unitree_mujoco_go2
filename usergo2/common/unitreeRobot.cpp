...
***********************************************************************/
#include "unitreeRobotgo2.h"
#include <iostream>

Vec3 QuadrupedRobot::getX(RobotInterface &robot_interface){
    return getFootPosition(state, 0, FrameType::BODY);
}

Vec34 QuadrupedRobot::getVecXP(RobotInterface &robot_interface){
    Vec3 x = getX(state);
    Vec34 vecXP, qLegs;
    qLegs = state.getQ();
...
// Forward Kinematics
Vec3 QuadrupedRobot::getFootPosition(RobotInterface &robot_interface, int id, FrameType frame){
...
Vec3 QuadrupedRobot::getFootVelocity(RobotInterface &robot_interface, int id){
    Vec34 qLegs = state.getQ();
...
// Forward Kinematics
Vec34 QuadrupedRobot::getFeet2BPositions(RobotInterface &robot_interface, FrameType frame){
    Vec34 feetPos;
...
Vec34 QuadrupedRobot::getFeet2BVelocities(RobotInterface &robot_interface, FrameType frame){
    Vec34 feetVel;
    for(int i(0); i<4; ++i){
...
Mat3 QuadrupedRobot::getJaco(RobotInterface &robot_interface, int legID){
...
Go2Robot::Go2Robot(){
    ...
    _feetPosNormalStand <<  0.00571868, -0.00571868,0.00571868, -0.00571868,
