...
template <typename USER_CTRL> //这个类可以接收任意类型作为 USER_CTRL
...
protected:
    ...
    SimpleStateMachine state_machine;  //状态机部分
    USER_CTRL ctrl;                    // 
...
public:
    ...
    void StartControl() 
    {
        ...
        control_thread_ptr = CreateRecurrentThreadEx("ctrl", UT_CPU_ID_NONE, ctrl_dt_micro_sec, &RobotController::ControlStep, this); //不断调用 ControlStep 函数来进行机器人控制的主要逻辑
