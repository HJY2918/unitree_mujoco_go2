...
void UnitreeSdk2Bridge::PublishLowState()
{
    if (mj_data_)
    {
        ...
        if (have_frame_sensor_)
        {
            ...
            // std::cout << "IMU: "
            //           << "quat: [" << low_state.imu_state().quaternion()[0] << ", " << low_state.imu_state().quaternion()[1] << ", " << low_state.imu_state().quaternion()[2] << ", " << low_state.imu_state().quaternion()[3] << "], "
            //           << "gyro: [" << low_state.imu_state().gyroscope()[0] << ", " << low_state.imu_state().gyroscope()[1] << ", " << low_state.imu_state().gyroscope()[2] << "], "
            //           << "acc: [" << low_state.imu_state().accelerometer()[0] << ", " << low_state.imu_state().accelerometer()[1] << ", " << low_state.imu_state().accelerometer()[2] << "]"
            //           << std::endl;
        }

        for(int i=0;i<4;i++)
        {
            low_state.foot_force()[i]=
            sqrt(mj_data_->sensordata[dim_motor_sensor_ + 16 + i*3 ]*mj_data_->sensordata[dim_motor_sensor_ + 16 + 3*i]+mj_data_->sensordata[dim_motor_sensor_ + 16 + i*3+1]*mj_data_->sensordata[dim_motor_sensor_ + 16 + 3*i+1]+mj_data_->sensordata[dim_motor_sensor_ + 16 + i*3+2]*mj_data_->sensordata[dim_motor_sensor_ + 16 + 3*i+2]);
        }
        // std::cout<<"foot force: "<<low_state.foot_force()[0]<<" "<<low_state.foot_force()[1]<<" "<<low_state.foot_force()[2]<<" "<<low_state.foot_force()[3]<<std::endl;
